...
async function throttledRouteAll(segments) {
  ...
  for (let i = 0; i < segments.length; i += CONCURRENCY) {
    ...
    for (let j = 0; j < batchResults.length; j++) {
      ...
      if (r.status === "fulfilled" && r.value) {
        console.log(`[OSRM] Route ${idx}: ${r.value.coords.length} pts, ${r.value.dist.toFixed(0)}m (straight: ${straightDist.toFixed(0)}m, ratio: ${(r.value.dist / straightDist).toFixed(1)}x)`);
        results[idx] = r.value;
        routeOk++;
      } else {
        const reason = r.status === "rejected" ? r.reason?.message || r.reason : "null response";
