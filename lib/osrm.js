...
export async function fetchRoutedShape(cacheKey, shapePoints) {
  ...
  // Step 5: Remove staircase bumps — small rectangular jogs from grid routing
  const smoothed = removeBumps(allCoords);
  const smoothedDist = computeTotalDist(smoothed);
  console.log(`[OSRM] Step 5: Bump removal: ${allCoords.length} → ${smoothed.length} coords (removed ${allCoords.length - smoothed.length})`);
  console.log(`[OSRM]   Total distance: ${(smoothedDist / 1609.34).toFixed(2)} mi (${smoothedDist.toFixed(0)} m)`);

  const entry = { coords: smoothed, dist: smoothedDist };
...
/**
 * Remove staircase bumps from the stitched route.
 * Detects places where the route jags out (e.g. one block east + one block south)
 * and returns close to where it was. Skips the bump to create a cleaner shape.
 */
function removeBumps(coords) {
  ...
  while (i < coords.length) {
    ...
    // Look ahead: does the route return close to this point within 3-8 steps?
    let skipTo = -1;
    for (let j = i + 3; j < Math.min(i + 9, coords.length); j++) {
      const loopDist = haversine(coords[i], coords[j]);
      // Calculate how far the route actually traveled in between
      let pathDist = 0;
      for (let k = i; k < j; k++) {
        pathDist += haversine(coords[k], coords[k + 1]);
      }
      // If the route traveled significantly but ended up close to start, it's a bump
      if (loopDist < 80 && pathDist > 150) {
        skipTo = j;
        break;
      }
    }
    ...
    if (skipTo > 0) {
      console.log(`[OSRM] Bump removed: coords ${i}→${skipTo} (loop: ${haversine(coords[i], coords[skipTo]).toFixed(0)}m, path: ${(() => { let d = 0; for (let k = i; k < skipTo; k++) d += haversine(coords[k], coords[k+1]); return d.toFixed(0); })()}m)`);
      i = skipTo;
    } else {
      i++;
    }
...
function computeTotalDist(coords) {
  let dist = 0;
