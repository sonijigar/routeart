...
function heartCurve(t) {
  ...
  // Clamp bottom to -0.7 to avoid sharp V that creates messy loops
  const ny = y / 17;
  return [x / 17, Math.max(ny, -0.7)];
}
...
export const SHAPES = {
  heart: {
    name: "Heart",
    icon: "♥",
    // Center of SLU — north enough that bottom V stays above Denny Way
    center: [47.6235, -122.3360],
    radius: 380,
    numPoints: 24,
  },
  lightning: {
